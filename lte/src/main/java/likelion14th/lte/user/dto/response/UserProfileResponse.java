...
public class UserProfileResponse{
    ...
    // 답변: 첫번째 이유는 보안 때문이다. Entity를 화면에 그래도 반환하면 불필요한 정보까지 노출될 수 있어 보안이 약해진다.
    // 두번째 이유는 API 스펙 보호 때문이다. DB 테이블에 column이 추가되거나 이름이 바뀌어도, DTO가 중간 방어막 역할을 하기 때문에
    // DTO에서 매핑하는 방식만 수정하면 프론트엔드에는 아무런 영향이 가지 않는다.
